...
// (XLA for open registration devices)
...
struct DummyCustomAllocator final : at::Allocator {
  ...
  at::DataPtr allocate(size_t nbytes) const override {
    ...
    return {data, data, &ReportAndDelete, at::Device(at::DeviceType::XLA, 0)};
  }
...
REGISTER_ALLOCATOR(c10::DeviceType::XLA, &global_custom_alloc);
...
struct DummyDeviceGuardImpl final : public c10::impl::DeviceGuardImplInterface {
  static constexpr c10::DeviceType static_type = c10::DeviceType::XLA;
  DummyDeviceGuardImpl() {}
  explicit DummyDeviceGuardImpl(c10::DeviceType t) {
    TORCH_INTERNAL_ASSERT(t == c10::DeviceType::XLA);
  }
  at::DeviceType type() const override { return at::DeviceType::XLA; }
  at::Device exchangeDevice(at::Device d) const override {
    TORCH_INTERNAL_ASSERT(d.type() == at::DeviceType::XLA);
    TORCH_INTERNAL_ASSERT(d.index() < deviceCount(), "Error: device index ",
                          d.index(), " does not exist.");
  ...
  at::Device getDevice() const override {
    return at::Device(at::DeviceType::XLA, CURR_DEVICE);
  }
  ...
  at::Stream exchangeStream(at::Stream) const noexcept override {
    // no-op
    return at::Stream(at::Stream::DEFAULT,
                      at::Device(at::DeviceType::XLA, CURR_DEVICE));
  }
  ...
  void record(void ** /*event*/, const at::Stream & /*stream*/,
              const at::DeviceIndex /*device_index*/,
              const c10::EventFlag /*flag*/) const override {
    TORCH_CHECK(false, at::DeviceType::XLA, " backend doesn't support events.");
  }
  ...
  void destroyEvent(void * /*event*/,
                    const at::DeviceIndex /*device_index*/) const
      noexcept override {}
...
C10_REGISTER_GUARD_IMPL(XLA, DummyDeviceGuardImpl);
...
//
// Note: this kernel is very simple because our "custom device" just uses the
// normal TensorImpl object to store data under the hood. In PyTorch core today,
// both cpu and cuda are implemented with an ordinary TensorImpl class.
// Sometimes, backends prefer to subclass TensorImpl in order to store extra
// information. If this is the case, then this kernel is where you'll be
// responsible for creating and returning a fresh at::Tensor object, that
// properly stores a TensorImpl of your subclass.
at::Tensor custom_empty_memory_format(
  ...
  constexpr c10::DispatchKeySet private_use_ks(c10::DispatchKey::XLA);
...
at::Tensor custom__copy_from(const at::Tensor &self, const at::Tensor &dst,
  ...
  TORCH_CHECK(self.is_cpu() || self.device().type() == c10::DeviceType::XLA,
              "Dummy test only allows copy from cpu -> dummy device.");
  TORCH_CHECK(dst.is_cpu() || dst.device().type() == c10::DeviceType::XLA,
              "Dummy test only allows copy from cpu -> dummy device.");
...
// For open registration, we're registering all of our kernels to the XLA
// dispatch key. Later in this file, we map a custom device to the XLA device
// type, which allows user code that puts a tensor on your custom_device to
// eventually get plumbed into the kernels registered here.
//
...
c10::Device get_custom_device(int idx) {
  return c10::Device(c10::DeviceType::XLA, idx);
}
...
// C++. The implementation in this file maps directly to the `XLA` device type.
