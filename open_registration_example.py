import torch
from utils.custom_device_mode import xla_module
...
# (Correctly) unable to create tensor on device='xla:2'
# Creating x on device 'xla:0'
# Custom aten::empty.memory_format() called!
# Custom allocator's allocate() called!
# Creating y on device 'xla:0'
# Custom aten::empty.memory_format() called!
# Custom allocator's allocate() called!

# Test START
# x.device=xla:0, x.is_cpu=False
# y.device=xla:0, y.is_cpu=False
# Calling z = x + y
# Custom aten::add.Tensor() called!
# Custom aten::empty.memory_format() called!
# Custom allocator's allocate() called!
# z.device=xla:0, z.is_cpu=False
...
# Creating x on device 'xla:1'
# Custom aten::empty.memory_format() called!
# Custom allocator's allocate() called!
# Creating y on device 'xla:1'
# Custom aten::empty.memory_format() called!
# Custom allocator's allocate() called!
...
# Option 1: Use torch.register_XLA_backend("xla"), which will allow
# "xla" as a device string to work seamlessly with pytorch's API's.
# You may need a more recent nightly of PyTorch for this.
# torch.register_XLA_backend('xla')
...
# Show that in general, passing in a custom device string will fail.
try:
    x = torch.ones(4, 4, device='xla:2')
    exit("Error: the xla device only has two valid indices: xla:0 and xla:1")
except RuntimeError as e:
    print("(Correctly) unable to create tensor on device='xla:2'")

print("Creating x on device 'xla:0'")
x1 = torch.ones(4, 4, device='xla:0')
print("Creating y on device 'xla:0'")
y1 = torch.ones(4, 4, device='xla:0')
...
xla_device1 = xla_module.custom_device(1)

print("Creating x on device 'xla:1'")
x2 = torch.ones(4, 4, device=xla_device1)
print("Creating y on device 'xla:1'")
y2 = torch.ones(4, 4, device=xla_device1)

# Option 3: Enable a TorchFunctionMode object in user land,
# that will convert `device="xla"` calls into our custom device objects automatically.
